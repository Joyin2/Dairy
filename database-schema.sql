...
-- Note: Update these policies based on your authentication setup
-- For development, you can temporarily disable RLS or create permissive policies

-- Example: Allow all operations for authenticated users (development only)
CREATE POLICY "dev_all_app_users" ON app_users FOR ALL USING (true);
CREATE POLICY "dev_all_milk_collections" ON milk_collections FOR ALL USING (true);
CREATE POLICY "dev_all_batches" ON batches FOR ALL USING (true);
CREATE POLICY "dev_all_deliveries" ON deliveries FOR ALL USING (true);
CREATE POLICY "dev_all_routes" ON routes FOR ALL USING (true);
CREATE POLICY "dev_all_ledger" ON ledger_entries FOR ALL USING (true);

-- ===== Triggers =====

-- Trigger: Create ledger entry on delivery
...
BEGIN
  IF (TG_OP = 'UPDATE' AND NEW.status = 'delivered') OR (TG_OP = 'INSERT' AND NEW.status = 'delivered') THEN
    IF COALESCE(NEW.collected_amount,0) > 0 THEN
      INSERT INTO ledger_entries (from_account, to_account, amount, mode, reference, receipt_url, created_at)
      VALUES (
